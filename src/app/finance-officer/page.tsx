  ...
  const fetchData = async () => {
    try {
      ...
      const studentsWithStipend: StudentWithStipend[] = studentsData.map((student: any) => {
        // Determine year level from date (simplified logic - you may need to adjust)
        const enrollmentYear = new Date(student.created_at).getFullYear();
        const currentYear = new Date().getFullYear();
        const yearLevel = `Year ${Math.min(currentYear - enrollmentYear + 1, 4)}`;
        ...
        return {
          ...
          status: 'active' as const, // You may need to add status logic
          stipendReceived: true, // Default - adjust based on your logic
      ...
      alert('Failed to fetch student data. Please check if the backend server is running at http://localhost:8080');
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <>
            ...
            {programsWithStudents.length === 0 && !isLoading && (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <h3 className="text-xl font-semibold text-gray-700 mb-2">No Student Data Available</h3>
                <p className="text-gray-500">Unable to fetch student data from the backend. Please check your connection.</p>
              </div>
            )}
